...
cost_file_path = r"consts\consts8.txt"
        ...
        for line in output.splitlines():
            if "Result" in line:
                if propertyNumber == 5:
                    ...
                    p_gas_detect_success = cost_group["p_gas_detect"]
                    print(f"Result for cost group {total_sections} , property { propertyNumber} : {line}")
                    if total_sections not in result_data:
                        result_data[total_sections] = {"p_gas_detect_success": [], "property_5": []}

                    result_data[total_sections]["p_gas_detect_success"].append(p_gas_detect_success)
...
plot_grouped_results(result_data, "plots/grouped_plot_results_gas.png")
